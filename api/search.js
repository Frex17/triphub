...
const BATCH = 10;             // scali processati in parallelo per volta
const TIME_BUDGET_MS = 55000; // limite di sicurezza: restituisce ciò che ha trovato prima di scadere
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const maxH = parseFloat(q.maxH || "6");
...
async function search(from, to, date, minH, maxH) {
  ...
  // processa gli scali a blocchi: TUTTI vengono esaminati, ma senza aprire centinaia di richieste insieme
  const started = Date.now();
  meta.timedOut = false;
  for (let i = 0; i < hubs.length; i += BATCH) {
    if (Date.now() - started > TIME_BUDGET_MS) { meta.timedOut = true; meta.hubsScanned = i; break; }
    await Promise.all(hubs.slice(i, i + BATCH).map(scanHub));
  }
  if (!meta.timedOut) meta.hubsScanned = hubs.length;
  ...
  const capped = options;   // nessun limite: tutte le combinazioni trovate
